...
func InitLog() {
	...
	file, err := os.OpenFile("tieba_sign.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	writers := []io.Writer{
		file,
		os.Stdout}
	//同时写文件和屏幕
...
func main() {
	...
	for idx, bduss := range config.BDUSS {
		...
		data := nic.KV{"msg_type": "text", "content": nic.KV{"text": fmt.Sprintf("签到成功，耗时%d秒", time.Since(start))}}
		nic.Post(config.Lark, nic.H{
			Data: data,
		})
	}
}
